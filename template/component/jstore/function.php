...
function featured_page(){
	...
	for($i=0;$i <= (count($fpg) - 1);$i++){
		...
		if($_fpg->featured_page['image']!=''){
			...
			$fp2[($_fpg->featured_page['order'] > 0 ? intval($_fpg->featured_page['order']) : $i.'999')] = array('ID' => $fpg[$i]->ID,'image' => $_fpg->featured_page['image'],'custom' => $custom);
		}
	}
	ksort($fp2);
	$idx = 0;
	...
	if(get_option('featured_page') == 'yes'){
		for($i=0;$i <= (count($fp3) - 1);$i++){
			$fp.='<div id="fp'.$fp3[$i]['ID'].'" class="slide'.($i == 0 ? ' slide_active' : '').'"><img src="'.$fp3[$i]['image'].'" />'.($fp3[$i]['custom'] !='' ? '<div class="custom">'.html_entity_decode($fp3[$i]['custom']).'</div>' : '').'</div>'."\n";
		}		
	}else{
		$i = rand(0,(count($fp3) - 1));
		$fp.='<div id="fp'.$fp3[$i]['ID'].'" class="slide slide_active"><img src="'.$fp3[$i]['image'].'" />'.($fp3[$i]['custom'] !='' ? '<div class="custom">'.html_entity_decode($fp3[$i]['custom']).'</div>' : '').'</div>'."\n";	
	}
